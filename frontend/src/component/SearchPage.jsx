  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([searchRes, topRes]) => {
        ...
        const searchIds = new Set(searchData.map((m) => m.id));
        const filteredRecommendations = topData.filter(
          (m) => !searchIds.has(m.id),
        );
  ...
  const MovieCard = ({ movie }) => (
    ...
    >
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-gray-900">
        <img
          src={movie.poster_url || movie.backdrop_url || "/banner.png"}
          alt={movie.title}
        ...
        <span className="absolute top-2 left-2 bg-red-600/90 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow">
          {movie.status || movie.current_episode || "FULL"}
        </span>
        <span className="absolute bottom-2 left-2 bg-sky-600/90 backdrop-blur-md text-white text-[9px] font-semibold px-1.5 py-0.5 rounded shadow">
          SUB+TM
        </span>
